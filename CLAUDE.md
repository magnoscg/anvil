...
- **Platform**: iOS **18.0+** Swift 6 XCODE ^26
...
| Setting | Value |
|---------|-------|
| Project | `Arquitectura.xcodeproj` |
| Scheme | `Arquitectura-Dev` |
| Bundle ID | `com.magnos.Arquitectura` |

### Quick Commands

> **UDID del simulador**: `B14005E0-AD25-4A48-BA38-6559F4684B8F` (iPhone 17 Pro)

| Acción | Comando |
|--------|---------|
| **Build** | `xcodebuild build -scheme Arquitectura-Dev -destination "platform=iOS Simulator,id=B14005E0-AD25-4A48-BA38-6559F4684B8F"` |
| **Unit Tests** | `xcodebuild test -scheme Arquitectura-Dev -destination "platform=iOS Simulator,id=B14005E0-AD25-4A48-BA38-6559F4684B8F"` |
| **Lanzar app** | `xcrun simctl launch booted com.magnos.Arquitectura` |
| **Screenshot** | Skill: `/axiom:screenshot` (preferido) o `axe screenshot --udid ...` |
| **Test Visual** | Skill: `/axiom:test-simulator` (escenarios completos con verificación) |
| **Describir UI** | `axe describe-ui --udid B14005E0-AD25-4A48-BA38-6559F4684B8F` |
| **Tap elemento** | `axe tap --label "texto" --udid B14005E0-AD25-4A48-BA38-6559F4684B8F` |
| **Swipe** | `axe swipe --start-x 200 --start-y 600 --end-x 200 --end-y 200 --udid B14005E0-AD25-4A48-BA38-6559F4684B8F` |

### Axiom Skills para Visual Testing

> **Usa Axiom skills primero** para testing visual. Son más inteligentes que comandos raw.

| Situación | Skill/Agent |
|-----------|-------------|
| Screenshot rápido | `/axiom:screenshot` |
| Test visual completo (escenarios, verificación) | `/axiom:test-simulator` |
| Debugging iterativo con screenshots | `axiom:simulator-tester` agent |

### Axiom Agents (Solo para problemas complejos)

| Situación | Agent |
|-----------|-------|
| Build falla repetidamente | `axiom:build-fixer` |
| Tests fallan sin razón clara | `axiom:test-debugger` |
| Tests flaky en CI | `axiom:test-failure-analyzer` |

### Axiom Skills Reference (Cuándo usar cada uno)

> **Consulta la documentación detallada** en `.claude/docs/` para cada área.

#### Pre-commit Audits (Ejecutar antes de PR)

| Situación | Skill | Doc |
|-----------|-------|-----|
| Nuevo código SwiftUI | `/axiom:audit swiftui-architecture` | `swiftui-code-style.md` |
| Código async/await nuevo | `/axiom:audit concurrency` | `swift-concurrency.md` |
| Cambios en persistencia | `/axiom:audit core-data` | `swiftdata.md` |
| Networking nuevo | `/axiom:audit networking` | `networking.md` |
| UI nueva | `/axiom:audit accessibility` | `design-system.md` |
| Código Codable/JSON | `/axiom:audit codable` | `new-feature.md` |
| Antes de release | `/axiom:audit security` | `security-privacy.md` |

#### Diagnóstico de Problemas

| Síntoma | Skill | Doc |
|---------|-------|-----|
| Build aleatorio falla | `axiom:build-fixer` agent | `diagnostics.md` |
| App lenta/jank | `/axiom:audit swiftui-performance` | `performance.md` |
| Memory warnings | `/axiom:audit memory` | `performance.md` |
| Battery drain | `/axiom:audit energy` | `performance.md` |
| Crash en producción | `/axiom:analyze-crash` | `diagnostics.md` |
| Build times lentos | `/axiom:optimize-build` | `diagnostics.md` |

#### Profiling y Performance

| Necesidad | Skill | Doc |
|-----------|-------|-----|
| Profile CPU/Memory | `/axiom:profile` | `performance.md` |
| SwiftUI view updates | `/axiom:audit swiftui-performance` | `performance.md` |
| Swift code hotspots | `/axiom:audit swift-performance` | `performance.md` |

#### Modernización

| Necesidad | Skill | Doc |
|-----------|-------|-----|
| Migrar a iOS 17/18 patterns | `/axiom:modernize` | `swiftui-code-style.md` |
| Actualizar a Liquid Glass (iOS 26+) | `/axiom:audit liquid-glass` | `design-system.md` |

### Apple Documentation Lookup

Use **sosumi MCP** to search and fetch official Apple documentation:

```bash
# Search Apple docs
mcp__sosumi__searchAppleDocumentation(query: "@Observable State SwiftUI")

# Fetch specific doc
mcp__sosumi__fetchAppleDocumentation(path: "/documentation/swiftui/state")
```

**When to use**: Before making architectural decisions, verify patterns with official Apple documentation.
...
|------|---------|---------|
| Domain Model | `<Feature>Model.swift` | `ArchitectureModel.swift` |
| UseCase Protocol | `<Feature>UseCase.swift` | `ArchitectureUseCase.swift` |
| UseCase Impl | `<Feature>UseCaseImpl.swift` | `ArchitectureUseCaseImpl.swift` |
| Repository Protocol | `<Feature>Repository.swift` | `ArchitectureRepository.swift` |
| Repository Impl | `<Feature>RepositoryImpl.swift` | `ArchitectureRepositoryImpl.swift` |
| Remote DataSource | `<Feature>RemoteDataSource.swift` | `ArchitectureRemoteDataSource.swift` |
| DTO | `<Feature>DTO.swift` | `ArchitectureDTO.swift` |
| DTOMapper | `<Feature>DTOMapper.swift` | `ArchitectureDTOMapper.swift` |
| View | `<Feature>View.swift` | `ArchitectureView.swift` |
| State | `<Feature>State.swift` | `ArchitectureState.swift` |
| Decorator | `<Feature>Decorator.swift` | `ArchitectureDecorator.swift` |
| ViewModel | `<Feature>ViewModel.swift` | `ArchitectureViewModel.swift` |
| DecoratorMapper | `<Feature>DecoratorMapper.swift` | `ArchitectureDecoratorMapper.swift` |
| Router | `<Feature>Router.swift` | `ArchitectureRouter.swift` |
| Optional RouteResolver | `<Feature>RouteResolver.swift` | `ArchitectureRouteResolver.swift` |
| Factory | `<Feature>Factory.swift` | `ArchitectureFactory.swift` |
...
- if does not exists Strings catalog, create it
...
- Features register their resolver in the app root only when that resolver exists: `.withArchitectureRoutes(appRouter:)`.
...
| `.claude/docs/workflow-qa.md` | Visual testing: "prueba visual", "probar en simulador", "screenshot" |
...
> Si algo es ambiguo, **preguntar antes de actuar**. No escribas codigo sin PRP aprobado.
...
| `/dev-design [desc]` | Crear disenos visuales en Paper/Figma |
| `/dev-build-yolo` | Auto-generar PRPs + ejecutar todas las features pendientes |
...
| `/dev-qa [target]` | QA visual y funcional con AXe |
| `/dev-profile [target]` | Perfilar feature con xctrace |
| `/dev-retro [N]` | Generar retrospectiva de una fase |
| `/dev-archive [slug]` | Archivar feature completada |
| `/dev-registry` | Ver el registro de skills disponibles |
| `/dev-registry-refresh` | Regenerar skill-registry desde todas las fuentes |
| `/dev-init` | Inicializar workflow (CLAUDE.md + .dev/ + plan/) |

### Flujo de trabajo

1. **Definir** -- `/dev-prd` para crear PRD en `plan/<feature>/PRD.md`
2. **Planificar** -- `/dev-plan` para generar PRPs en `plan/<feature>/prp-*.md`
3. **Disenar** -- `/dev-design` para mockups en Paper/Figma (opcional)
4. **Construir** -- `/dev-build` para ejecutar fase a fase
5. **Verificar** -- `/dev-status` para ver progreso
6. **QA Visual** -- `/dev-qa` para inspeccion visual con screenshots
7. **Perfilar** -- `/dev-profile` para analisis de rendimiento
8. **Buscar** -- `/dev-search` para encontrar PRDs/PRPs de otros proyectos
9. **Auditar** -- `/dev-verify` para verificacion arquitectonica
10. **Fast-forward** -- `/dev-ff` para bugfixes rapidos (PRD+plan+build en un paso)
...
- Checkboxes: `- [x]` completado, `- [!]` bloqueado.
